...
import com.anew.devl.cardealershipmanager.fipeclient.MarcaMotoSelectActivity;
...
//                        .setAction("Action", null).show();
    ...
    private void startBuscaMotoFipe() {
        if (Utils.isOnline(getBaseContext())) {
            Intent in = new Intent(getBaseContext(), MarcaMotoSelectActivity.class);
            startActivity(in);
        } else {
            Toast.makeText(getApplicationContext(), "Não há conexão com a Internet", Toast.LENGTH_LONG).show();
        }
